...
let articles = bumplayoutFooter.topArticles
let nightmare

harPlugin.install(Nightmare)

describe('Click Top Articles Section On Footer',function(){
  this.timeout('300s')
  ...
  Object.keys(articles).forEach(key => {
    ...
    it(`should fire event track when click ${articles[key]['selection']}`,function(done){
      nightmare
        .waitForDevtools()
        .goto(homepageUrl)
        .wait(3000)
        .wait(articles[key]['locator'])
        .resetHAR()
        .click(articles[key]['locator'])
        .wait(5000)
        .getHAR()
        .end()
        .then((result)=>{
          ...
          let selectionFound = searchHAR.findProperty(result.entries, 'selection', `articles > ${articles[key]['selection']}`)
